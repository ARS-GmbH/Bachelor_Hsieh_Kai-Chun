...
class AzureCustomVision(interface.SolutionManager):
    ...
    def createModel(self, database, id):
        ...
        session.execute("INSERT INTO " + self._datatableName + " (id, remote_id) VALUES (%s, %s)", (id, project.id))
        database.commit()
    ...
    def feedTrainData(self, database, id, newTrainData):
        ...
        session = database.cursor()
        session.execute("SELECT remote_id, training_data, extra_info FROM " + self._datatableName + " WHERE id = %s", (id,))
        result = session.fetchone()
        session.close()

        if result is None:
            print("INFO (", self._datatableName,") Model", id, "can no longer be found from the plugin database.")
        ...
        # update record
        session = database.cursor()
        session.execute("UPDATE " + self._datatableName + " SET training_data=%s WHERE id=%s;", (json.dumps(trainingData), id))
        database.commit()
    ...
    def trainModel(self, database, modelID, parameters, onMessage, onFinished):
        ...
        session = database.cursor()
        session.execute("SELECT remote_id, training_data, extra_info FROM " + self._datatableName + " WHERE id = %s", (modelID,))
        result = session.fetchone()
        session.close()
        ...
        if result:
            ...
            try:
                ...
                for photoID in trainingData:
                    ...
                    if err:
                        imageFailed += 1
                        onMessage("Failed to download image " + str(photoID) + ". Error: " + err + " " + visualizeImageDownload())
                    else:
                        imageOK += 1
                        ...
                        imageList.append(ImageFileCreateEntry(name=str(photoID) + ".png", contents=encodedImage, tag_ids=[dataClassList[classOfData].id]))
    ...
    def predictWithID(self, database, modelID, inputDataIDs, onFinished):
        ...
        # load model record
        session = database.cursor()
        session.execute("SELECT remote_id FROM " + self._datatableName + " WHERE id = %s", (modelID,))
        result = session.fetchone()
        session.close()
        ...
        if result:
            ...
            if len(inputDataIDs) > 0:
                try:
                    ...
                    trainer = CustomVisionTrainingClient(self._trainingKey, endpoint=self._endPoint)
                    predictor = CustomVisionPredictionClient(self._predictionKey, endpoint=self._endPoint)
                    project = trainer.get_project(projectID)
                    ...
                    for photoID in inputDataIDs:
                        ...
                        if err is None:
                            isOK, encodedImage = cv2.imencode('.png', image)
                            predictResponse = predictor.classify_image(project.id, projectID, encodedImage)
                            predictResult = predictResponse.predictions
                            if len(predictResult) > 0:
                                resultMap[photoID] = {"CLASS": predictResult[0].tag_name, "SCORE": predictResult[0].probability}

                except Exception as err:
                    predictOK = False
                    resultMap = {}
                    print("ERROR (", self._datatableName,") Model", modelID, "failed to predict: " + str(err))
            ...
            print("ERROR (", self._datatableName,") Model", modelID, "failed to predict, model record not found by plugin.")
