...
class LocalFoamEliminationDetector(interface.SolutionManager):
    ...
    def createModel(self, database, id):
        ...
        session.execute("INSERT INTO " + self._datatableName + " (id, remote_id) VALUES (%s, %s)", (id, MODEL_ID_PREFIX + "-" + str(int(time.time_ns())) + ".model"))
        database.commit()
    ...
    def trainModel(self, database, modelID, parameters, onMessage, onFinished):
        ...
        if result:
            ...
            if imageOK < 2:
                onMessage("Model training aborted.  Number of successfully downloaded training data is not enough.")
                onFinished(False)
                return
        ...
        if result:
            ...
            if len(inputOpenCVImages) > 0:
                ...
                for index in range(len(predictResult)):
                    resultMap[imageNames[index]] = {"CLASS": str(predictResult[index]), "SCORE": 1.0}
    ...
    def _predict(self, modelFileName, opecvPhotos):
        ...
        try:
            ...
            print("ERROR (", self._datatableName,") Model", modelFileName, "failed to be loaded, debug message: " + str(err))
            return None
