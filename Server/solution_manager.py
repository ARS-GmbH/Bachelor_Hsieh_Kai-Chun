...
# imports
import os, importlib, utils, queue, threading
...
def initialize(database, app, serverAPI):
    ...
    for plugin in os.listdir("./" + SOLUTION_PLUGINS_PATH):
        ...
        if pluginFilename.endswith(".py") and not pluginFilename.startswith("interface") and not pluginFilename.startswith("__init__"): 
            pluginModule = importlib.import_module(SOLUTION_PLUGINS_PATH + "." + pluginFilename[:-3])
            
            try:
                pluginClass = pluginModule.reflector()
            except:
                print("ERROR Solution plugin", pluginFilename, "was refused to load: ERR_NO_REFLECTOR_METHOD")
                continue

            try:
                instanceID = utils.getPluginID(pluginClass)
            except:
                print("ERROR Solution plugin", pluginFilename, "was refused to load: ERR_METADATA_INVALID")
                continue
            ...
            try:
                ...
                print("ERROR Solution plugin", pluginFilename, "was refused to load: ERR_DATATABLE_CREATE_FAILED")
                continue

            solutionPlugins[instanceID] = {"instance": pluginClass(databaseID, serverAPI), "class": pluginClass}
    ...
    def modelCreater():
        ...
        if solutionID is None:
            ...
            session = database.cursor()
            session.execute("INSERT INTO models (id, nickname, created_at, create_by, plugin_id, state, description) VALUES (%s, %s, NOW(), %s, %s, %s, %s);", 
                                                 (newID, nickName, "webuser0", solutionID, STATE_MODEL_CREATED, description))
    ...
    def modelInfoProvider():
        session = database.cursor()
        session.execute("SELECT id, nickname, created_at, create_by, plugin_id, state, description FROM models ORDER BY id DESC;")
    ...
    def feedModelTrainData():
        ...
        session.execute("SELECT id, plugin_id, state FROM models WHERE id=%s OR nickname=%s;", (queryModelID, queryModelNickName))
        ...
        if modelState > STATE_DATA_FEEDING:
            return "Model with state " + stateID2State[modelState] + " is no longer allowed to give training data.", 400
        
        result, err = solutionPlugins[pluginID]["instance"].feedTrainData(database, modelID, trainingData)
        if result:
            # update model state
            session = database.cursor()
            session.execute("UPDATE models SET state=%s WHERE id=%s;", (STATE_DATA_FEEDING, modelID))
            database.commit()
    ...
    def modelTrainer():
        ...
        def onFinished(isSucceed):
            if isSucceed:  
                # update model state to training
                session = database.cursor()
                session.execute("UPDATE models SET state=%s WHERE id=%s;", (STATE_MODEL_USABLE, modelID))
                database.commit()
            ...
            for file in files:
                if file and utils.isFileAllowed(file.filename, ALLOWED_EXTENSIONS):
                    ...
                    if mimetype.startswith("image/"):
                        # load image 
                        opencvImage = cv.imdecode(np.fromstring(file.read(), np.uint8), cv.IMREAD_COLOR)
                        ...
                        print("DEBUG Resource", file.filename,"upload rejected. Not supported currently.")
