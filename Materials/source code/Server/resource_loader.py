    ...
    def dataUploader():
        if request.method == 'POST':
            ...
            if pluginName and (pluginName in resourcePlugins):
                ...
                return jsonify(
                    {"OK": uploadOK, "FAILED": uploadFailed, "NOT-ALLOWED": uploadNotallowed}), 200
            else:
...
def getResource(database, resourceID):
    ...
    session = database.cursor()
    session.execute(
        "SELECT created_at, create_by, mime, plugin_id FROM resources WHERE id=%s",
        (resourceID,
         ))
    ...
    if result:
        ...
        if extraInfo:
            return {"CREATED-AT": createdAt, "CREATED-BY": createBy,
                    "PLUGIN-ID": pluginID, "EXTRA-INFO": extraInfo, "MIME": mime}, None
        else:
