...
class Model:
    ...
    def feed_train_data(self, resourceList, dataClass):
        ...
        res = requests.post("http://" +
                            ...
                            "/feed_train_data", params={"modelID": self._modelID}, json=dataset)
    ...
    def predictWithResourceList(self, resourceList, view=None):
        ...
        res = requests.post("http://" + self._serverAddress + ":" + str(self._serverPort) +
                            "/predict_w_list", params={"modelID": self._modelID}, json=resourceList.getIDs())
    ...
    def predictWithResources(self, resourceFilenameList, view=None):
        ...
        res = requests.post("http://" +
                            ...
                            ":" +
                            str(self._serverPort) +
                            "/predict", files=files, data={"modelID": modelName})
                ...
                if predictResult:
                    if len(predictResult.keys()) != 1:
                        ...
                        if triggerMode == SCORE:
                            ...
                            if scoreRaw >= scoreThreshold and sortedToClass == str(
                                    targetPredictionClass):
                                triggerCounter += 1
                            else:
                                triggerCounter = 0
                                ...
                                try:
                                    with open(tempFileName, "rb") as imageFile:
                                        ...
                                        opencvImage = cv.imdecode(np.fromstring(
                                            imageFile.read(), np.uint8), cv.IMREAD_COLOR)
                                        _tagPhoto(opencvImage, "Class: " + tag + "(" + score + ")")

                                        _, buffer = cv.imencode(".png", opencvImage)
                                        notificator.sendMessage(
                                            notification.MESSAGE_TYPE_IMAGE, io.BytesIO(buffer))
                                except Exception as err:
                                    raise RuntimeError(
                                        "Failed to read saved temp image file: " + tempFileName + str(err))
                            else:
                                notificator.sendMessage(
                                    notification.MESSAGE_TYPE_TEXT,
                                    "Target class '" + tag + "'(" + score + ") detected!")
...
def _toTableView(predictionResult):
    ...
    for imageID in predictionResult.keys():
        tag, score = _toImageTag(predictionResult[imageID]
                                 ["CLASS"], predictionResult[imageID]["SCORE"])
...
def _tagPhoto(opencvImage, tag):
    ...
    cv.rectangle(opencvImage, (10 - 10, 40 + 10),
                 (10 + textsize[0] + 10, 40 + 10 - textsize[1] - 20), (0, 0, 0), -1)
    ...
    for photoID in predictionResult:
        ...
        if isID:
            ...
            if res.status_code == 200:
                ...
                raise RuntimeError("Cache image " + str(photoID) +
                                   " failed with code: " + str(res.status_code))
        ...
        if image is not None:
            tag, score = _toImageTag(
                predictionResult[photoID]["CLASS"], predictionResult[photoID]["SCORE"])
